...
# Check here that there is only one result

if (length(concept) != 1) {
  stop("More than one record found! Update query to only return a single result!")
}
# Double-check that concept id & DOI matches and error if it doesn't
if (concept[[1]]$conceptdoi != "10.5281/zenodo.10823037") {
  stop("DOI of the record returned by the query isn't correct!\n",
       "Query returned ", concept[[1]]$conceptdoi, " not ", "10.5281/zenodo.10823037")
}
...
#zip the whole dir
#this is done in the temp directory so that the zip file doesn't contain nexted folders of the entire file path
withr::with_dir(dir, {
