...
#TODO: create air_temperature_f.value and wind_speed_mph.value columns?
new_dat <-
  gsi_get_data(start = prev_end) |> 
  #calculate "feels like" with wind chill and heat index
   mutate(
     # wind_chill.value = calc_wind_chill(air_temperature_f.value, wind_speed_mph.value),
     # heat_index.value = calc_hi(air_temperature.value, vapor_pressure.value),
     feels_like.value = calc_hi(air_temperature.value, vapor_pressure.value) |> 
       calc_wind_chill(wind_speed.value) #TODO current version needs units converted to F and mph.  Do this in calc_wind_chill()?
   )

#TODO calculate wind chill and adjusted temp on full data set ONCE
# bind old to new
all_dat <- 
